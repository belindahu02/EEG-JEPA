...
def trainer(num_users, fet_extrct, scen, ft, checkpoint_dir=None):
    """
    Train classifier with multi-GPU support using all available samples
    ...
    with strategy.scope():
        ...
        # Set trainability
        for i in range(1, min(ft + 1, len(fet_extrct_dist.layers))):
            fet_extrct_dist.layers[i].trainable = False
        ...
        # Adjust learning rate based on number of users and fine-tuning
    ...
    print(f"\nInitial learning rate: {initial_lr:.6f}")
    print(f"Training steps per epoch: {steps['train']}")
    print(f"Validation steps: {steps['val']}")
