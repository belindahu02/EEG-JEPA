...
def trainer(num_users, fet_extrct, scen, ft, checkpoint_dir=None):
    ...
    with strategy.scope():
        ...
        # Paper: Classifier with two hidden Dense layers (256, 64)
        inputs = Input(shape=(frame_size, n_channels))
        x = fet_extrct_dist(inputs, training=False)
        
        print(f"\nBackbone output shape: {x.shape}")
        
        # Paper architecture: Dense(256, ReLU) → Dense(64, ReLU) → Dense(num_classes, softmax)
        ...
        # Paper: Initial learning rate 0.01 for classifier with exponential decay
        total_steps = steps['train'] * 30  # 30 epochs
