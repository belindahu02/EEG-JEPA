...
def DA_SpecMask(spec, num_time_masks=2, num_freq_masks=2, max_time_mask=0.1, max_freq_mask=0.1):
    spec = spec.copy()
    T, F = spec.shape

    for _ in range(num_time_masks):
        t = np.random.randint(0, int(T * max_time_mask))
        t0 = np.random.randint(0, T - t)
        spec[t0:t0 + t, :] = 0

    for _ in range(num_freq_masks):
        f = np.random.randint(0, int(F * max_freq_mask))
