"""
EEG CSV to log-mel spectrogram (LMS) converter with 20s frames and 10s overlap.
...
Example:
    python eeg_csv_to_lms.py /path/to/combined /path/to/output --verbose=True
"""
import numpy as np
...
def convert_eeg_batch(base_from_dir, base_to_dir,
    ...
    """
    Convert EEG CSV files to log-mel spectrograms with 20s frames and 10s overlap.
    ...
    print(f'Processing RAW channels (RAW_TP9, RAW_AF7, RAW_AF8, RAW_TP10) with 20s frames and 10s overlap')
    ...
    print(f'\nFinished processing. Successfully processed {len(successful_files)}/{len(csv_files)} files.')
